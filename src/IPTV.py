...
class IPTV:
    def __init__(self, base_url, mac, sn="", device1="", device2="", signature="", post_url="", export=False):
        ...
        self.hostname   = base_url.split("://", 1)[-1].split("/",1)[0].split(":", 1)[0]
        if self.base_url[-3:] == "/c/":
    ...
    def gen_jsondata(self, jsondata, construct={}, validate=dict):
        data = jsondata.json().get("js", construct)
        if isinstance(data,validate):
            return data
        return construct
    
    def gen_scraper(self, payload, url=""):
        if url=="":
            url = self.portal_url
        r = self.scraper.post(url, data=payload)
        r.raise_for_status()
        if r.text.startswith("Authorization failed"):
    ...
    def gen_input(self):
        if self.sn!="" and self.device2!="" and self.signature!="" and self.post_url!="":
            return False
        if self.sn == "":
    ...
    def get_version(self):
        url = f"{self.base_url}/c/version.js"
        r = self.scraper.get(url)
        r.raise_for_status()
        match = re.search(r"var ver = '([^']+)'", r.text)
        self.version = match.group(1)
        print("→ version.js fetched, portal version =", self.version)
        self.gen_logs(r, "version.js")
        return self.version
    
    def get_xpcom(self):
        r = self.scraper.get(f"{self.base_url}/c/xpcom.common.js")
        r.raise_for_status()
        print("→ xpcom.common.js fetched, length =", len(r.text), "bytes")
        self.gen_logs(r, "xpcom.common.js")
        return r.text
    
    def get_initial(self, dsh=True):
        if dsh: print(self.dsh)
        print("GET initial request for initialisation files")
        self.get_version()
        self.get_xpcom()
        if dsh: print(self.dsh)
    ...
    def post_initial(self, dsh=True):
        if dsh: print(self.dsh)
        print("POST initial request for initialisation response")
        self.post_handshake()
        self.post_get_profile()
        self.post_get_main_info()
        if dsh: print(self.dsh)
    ...
    def get_itv_genres(self):
        ...
        self.gen_logs(r, "get_itv_genres.js", True)
    ...
    def get_vod_categories(self):
        ...
        self.gen_logs(r, "get_vod_categories.js", True)
    ...
    def get_series_categories(self):
        ...
        self.gen_logs(r, "get_series_categories.js", True)
    ...
    def get_all_channels(self):
        ...
        self.gen_logs(r, "get_all_channels.js", True)
        return self.all_channels
    
    def get_database(self, dsh=True):
        if dsh: print(self.dsh)
    ...
    def load_profile(self,db=None):
        ...
        print(self.dsh)
        self.get_initial(dsh=False)
        print(self.dsh)
        self.post_initial(dsh=False)
        print(self.dsh)
        if db is True:
            self.get_database(dsh=False)
            print(self.dsh)
        elif db is None:
            self.get_all_channels()
            print(self.dsh)
        print(f"{self.hostname} profile successfully loaded")
        print(self.dsh)
        print(self.dsh)
    ...
    def search_channel(self, query):
        print(self.dsh)
        print(f"Searching for {query} in all channel names:-")
        query = query.lower()
        matches = [channel for channel in self.all_channels.values() if query in channel["name"].lower()]
...
# INITIAL CODE written as: STBClient.py